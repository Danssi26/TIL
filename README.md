...
# 24.01
<details>
<summary>24.01.27</summary>
<div markdown="1">

**DE STUDY 강의**
* 경쟁력 있는 분석가를 위한 데이터 엔지니어링 기초 : AWS, GCP 기반 클라우드 분석 환경 마스터 (4.2~ 완강)
...
**데이터 엔지니어링 스터디**
* 빅데이터를 지탱하는 기술 ~ Cha 2.4
* 경쟁력있는 데이터 분석가를 위한 데이터 분서 기초 ~ 1.3
...
**`월스트리트저널 인포그래픽 가이드` - 파이차트**
* 사람들은 위에서 아래로, 시계 방향으로 읽는다.   
그렇기에 시계 방향으로 크기가 가장 작은 파이부터 가장 큰 파이 순으로 절대 배치 X
* 가장 작은 파이부터 가장 큰 파이 순으로 배치할 시 중요도가 가장 떨어지는 부분이 가장 눈에 띄는 위치에 오게 됨
* 파이 조각은 5조각 이내로
* 12시 정각을 기준으로 오른쪽에 가장 큰 파이 조각 배치, 그리고 왼쪽에 두번째 파이조각 배치
...
* 막대 뒤쪽에 그림자 효과 주지 말기. 막대 차트는 예술 작품이 아니다
* 막대마다의 제각기 다른 음영은 집중을 방해한다
* 3차원 수직 막대는 독자가 그리드 선에 닿는 입체 막대 끝부분이 어디인지 추측해야만 하기 때문에 비추
* 막대 너비는 막대와 막대 사이 간격의 약 두 배 크기로 잡기
* 한 차트에 있는 모든 막대의 색과 음영은 하나로 통일 
